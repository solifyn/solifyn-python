# coding: utf-8

"""
    Solifyn API

    Welcome to the Solifyn API Reference. Leverage our secure endpoints to manage products and issue, validate, and manage software license keys programmatically.

    The version of the OpenAPI document: 1.0.0
    Contact: support@solifyn.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from solifyn.models.webhook_license_payload import WebhookLicensePayload

class TestWebhookLicensePayload(unittest.TestCase):
    """WebhookLicensePayload unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> WebhookLicensePayload:
        """Test WebhookLicensePayload
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `WebhookLicensePayload`
        """
        model = WebhookLicensePayload()
        if include_optional:
            return WebhookLicensePayload(
                id = 'lic_hyQQhhNzfEuvv',
                key = 'A184-99C2-8CBE',
                status = 'ACTIVE',
                business_id = 'biz_345TUESrCDPvrb',
                product_id = 'prod_hyQQhhNzfEuvv',
                payment_id = 'pay_3r2FgHXp9',
                customer_id = 'cust_3r2FgHXp9',
                activation_limit = 3,
                activation_message = 'Thank you! Your license is now active.',
                instances_count = 1,
                expiry_hours = 720,
                expires_at = '2027-01-01T00:00:00.000Z',
                filters = None,
                archived = False,
                created_at = '2026-05-11T01:12:25.764Z',
                updated_at = '2026-05-18T09:45:37.166Z'
            )
        else:
            return WebhookLicensePayload(
                id = 'lic_hyQQhhNzfEuvv',
                key = 'A184-99C2-8CBE',
                status = 'ACTIVE',
                business_id = 'biz_345TUESrCDPvrb',
                product_id = 'prod_hyQQhhNzfEuvv',
                payment_id = 'pay_3r2FgHXp9',
                customer_id = 'cust_3r2FgHXp9',
                activation_limit = 3,
                activation_message = 'Thank you! Your license is now active.',
                instances_count = 1,
                expiry_hours = 720,
                expires_at = '2027-01-01T00:00:00.000Z',
                filters = None,
                archived = False,
                created_at = '2026-05-11T01:12:25.764Z',
                updated_at = '2026-05-18T09:45:37.166Z',
        )
        """

    def testWebhookLicensePayload(self):
        """Test WebhookLicensePayload"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
