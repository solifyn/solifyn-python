# coding: utf-8

"""
    Solifyn API

    Welcome to the Solifyn API Reference. Leverage our secure endpoints to manage products and issue, validate, and manage software license keys programmatically.

    The version of the OpenAPI document: 1.0.0
    Contact: support@solifyn.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from solifyn.models.subscription_action import SubscriptionAction

class TestSubscriptionAction(unittest.TestCase):
    """SubscriptionAction unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> SubscriptionAction:
        """Test SubscriptionAction
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `SubscriptionAction`
        """
        model = SubscriptionAction()
        if include_optional:
            return SubscriptionAction(
                free_days = 30,
                cancellation_mode = 'immediate',
                void_payments = True,
                addon_product_id = 'prod_seat_123',
                new_quantity = 5,
                proration_type = 'do_not_bill',
                idempotency_key = 'sub_action_idem_123'
            )
        else:
            return SubscriptionAction(
        )
        """

    def testSubscriptionAction(self):
        """Test SubscriptionAction"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
